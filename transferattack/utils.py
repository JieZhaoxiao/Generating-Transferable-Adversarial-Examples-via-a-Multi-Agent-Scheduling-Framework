...
def load_pretrained_model(cnn_model=[], vit_model=[]):
    ...
    for model_name in vit_model:
        local_weight_path = _local_timm_weight_path(model_name)
        if local_weight_path is not None:
            yield model_name, timm.create_model(
                model_name,
                pretrained=True,
...
def _local_timm_weight_path(model_name):
    """Prefer local timm weights so evaluation is not blocked by unstable HF access."""
    manual_dir = os.path.join('pretrained', 'timm', model_name)
    for filename in ('model.safetensors', 'pytorch_model.bin', 'model.pth'):
    ...
    repo_id = {
        ...
        'pit_b_224': 'timm/pit_b_224.in1k',
        'visformer_small': 'timm/visformer_small.in1k',
        'swin_tiny_patch4_window7_224': 'timm/swin_tiny_patch4_window7_224.ms_in1k',
        'deit_base_patch16_224': 'timm/deit_base_patch16_224.fb_in1k',
        'deit_base_distilled_patch16_224': 'timm/deit_base_distilled_patch16_224.fb_in1k',
    }.get(model_name)
    if repo_id is None:
        return None
...
def save_images(output_dir, adversaries, filenames):
    adversaries = adversaries.detach().permute((0, 2, 3, 1)).cpu().numpy()
    adversaries = np.rint(np.clip(adversaries, 0.0, 1.0) * 255).astype(np.uint8)
